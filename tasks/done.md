...
## F30 — TOTP for admin (enroll + verify + login enforcement) ✓ (recovery codes deferred to F30-recovery)
- **Tests:** api 190/190 (12 new in `test_auth_totp.py`), ruff clean. Live api smoke-tested: admin enrollment returns a usable `otpauth://` URI.
- **Files:**
  - `pyproject.toml` — adds `pyotp` dependency.
  - `src/tikko/models/user_totp.py` (new) — `UserTOTP(user_id PK + FK, secret_b32, enabled, created_at)`. 1:1 with `User`; using the FK as the primary key avoids a separate unique constraint.
  - `src/tikko/models/__init__.py` + `main.py` register `UserTOTP`.
  - `src/tikko/schemas/totp.py` (new) — `TOTPEnrollResponse`, `TOTPVerifyRequest/Response`, `TOTPDisableRequest`.
  - `src/tikko/schemas/user.py` — `LoginPayload.totp_code: str | None` (regex `^\d{6}$`).
  - `src/tikko/routes/totp.py` (new) — `POST /auth/totp/enroll`, `/verify`, `/disable`.
  - `src/tikko/routes/auth.py` — `POST /auth/login` now demands `totp_code` when `user.role == "admin"` and `totp.enabled`. Wrong/missing code returns 401 with `detail="totp_required"` or `"invalid totp code"`.
  - `alembic/versions/b2d4cc7dbe00_user_totp.py` — autogenerated, applied cleanly (table-create only; no batch_alter trap because it's a new table). Live dev DB at `b2d4cc7dbe00`.
- **Why TOTP only on admin login:** the F30 description says "TOTP for admin role". Employees and managers can still enroll (the same endpoints work) — useful if we later widen enforcement — but their login path doesn't gate on it. Keeps the rollout small.
- **Why `valid_window=1` on both `verify` and `login`:** RFC 6238 codes step every 30s. Without a 1-step window, a slight clock drift between the server and the user's phone causes spurious failures. The window accepts the previous + next code, which is the standard practical tradeoff between security and usability.
- **Why re-enroll rotates the secret (when not yet enabled):** if a user starts enrolling, loses their QR, and hits `/enroll` again, the second call should hand them a fresh secret rather than the abandoned one. Already-enabled users hit a 409 instead — they have to `/disable` first, which requires their password.
- **Why `/disable` re-checks the password:** disabling 2FA is destructive (loses the second factor). Demanding the password again is the standard pattern; otherwise a stolen access token alone could turn off the admin's 2FA.
